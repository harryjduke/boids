...
// NOTE: This function also takes in a pointer to a float that can be used to sum the collision time of the boids, the
// collision time of the given boid is added to the given float.
...
void UpdateFlock(struct FlockState *flockState) {
    ...
    for (int i = 0; i < flockState->boidsCount; i++) {
        Boid *boid = &flockState->boids[i];

        boid->velocity = Vector2Add(boid->velocity, CalculateSteeringVector(i, flockState, &flockState->collisionRate));

        UpdateBoidPosition(boid, flockState);
    }
}
